...
export async function sendContact(
  ...
  if (!emailRegex.test(email)) return { ok: false, error: 'Please enter a valid email.' }

  const apiKey = process.env.RESEND_API_KEY
  const to = process.env.CONTACT_TO
  const from = process.env.CONTACT_FROM
  if (!apiKey || !to || !from) return { ok: false, error: 'Email service is not configured.' }
  ...
  <div style="font-family: ui-monospace, SFMono-Regular, Menlo, Consolas, monospace; line-height:1.6">
    ...
    <pre style="white-space:pre-wrap; background:#0b0b0b; padding:12px; border-radius:8px; border:1px solid rgba(255,255,255,.1); color:#e5e5e5; margin-top:12px">${escapeHtml(message)}</pre>
  </div>`
  ...
  try {
    ...
    if (error) return { ok: false, error: 'Failed to send. Please try again later.' }
