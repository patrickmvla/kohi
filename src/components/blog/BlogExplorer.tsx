...
export default function BlogExplorer({ posts }: { posts: Post[] }) {
  ...
  const filtered = useMemo(() => {
    ...
    let list = posts.filter(p => {
      const inText = !query || norm(p.title).includes(query) || norm(p.excerpt).includes(query) || p.tags?.some(t => norm(t).includes(query))
      const inTags = tags.size === 0 || p.tags?.some(t => tags.has(t))
